...
int main() {
    ...
    if(filas == tamano && columnas == tamano) {
        int matriz_a3[tamano][tamano], matriz_b3[tamano][tamano];
        
        for(int i = 0; i < tamano; i++) {
            for(int j = 0; j < tamano; j++) {
                matriz_a3[i][j] = matriz_a[i][j];
                matriz_b3[i][j] = matriz_b[i][j];
            }
        }
        
        int deterA = determinante(matriz_a3);
        int deterB = determinante(matriz_b3);
