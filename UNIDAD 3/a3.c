...
Fraccion simplificar(Fraccion f) {
    int a = abs(f.numerador), b = abs(f.denominador), r;
...
void mostrar_unaria(char nombre, Fraccion** original, Fraccion** resultado, int filas, int columnas, int color_original, int color_resultado) {
    printf("\nMatriz %c:\n", nombre);
    mostrar_matriz_color(original, filas, columnas, color_original);
    printf("\nResultado:\n");
    mostrar_matriz_color(resultado, filas, columnas, color_resultado);
}
...
int menu_principal() {
    ...
    gotoxy(30, 13); printf("0. Salir");
    gotoxy(30, 15); printf("Seleccione una opcion: ");
    return leer_entero("");
}
        ...
        switch (opc) {
            ...
            case 2:
                if (f > 0 && c > 0) {
                    ...
                    mostrar_unaria('A', A, At, f, c, 10, 11);
                    liberar_matriz(At, c);
                }
                if (fB > 0 && cB > 0) {
                    Fraccion** Bt = transpuesta(B, fB, cB);
                    mostrar_unaria('B', B, Bt, fB, cB, 12, 11);
            ...
            case 7:
                if (f == c) {
                    ...
                    if (invA) {
                        mostrar_unaria('A', A, invA, f, f, 10, 11);
                ...
                if (fB == cB) {
                    ...
                    if (invB) {
                        mostrar_unaria('B', B, invB, fB, fB, 12, 11);
            ...
            case 9:
                if (f == fB && c == fB && cB == 1) {
                    ...
                    if (X) {
                        mostrar_operacion(A, B, X, f, 1, '*');
                        liberar_matriz(X, f);
                    } else {
                        mostrar_error_y_pausar("No se puede resolver el sistema, A no tiene inversa.");
                    }
                } else {
                    mostrar_error_y_pausar("AX = B solo es válido si A es cuadrada y B es una columna.");
                }
                break;
            case 0:
                despedida();
