...
Fraccion** resolver_sistema(Fraccion** A, Fraccion** B, int n) {
    Fraccion** invA = inversa(A, n);
    if (!invA) return NULL;
    Fraccion** X = multiplicar_matrices(invA, n, n, B, n, 1);
    liberar_matriz(invA, n);
    return X;
}
int menu_principal() {
    limpiar_pantalla();
    ...
    gotoxy(30, 11); printf("8. Adjuntas");
    gotoxy(30, 12); printf("9. Trazas");
    gotoxy(30, 13); printf("10. Resolver sistema AX = B");
        ...
        switch (opc) {
            ...
            case 3:
                if (f == fB && c == cB) {
                    ...
                    mostrar_error_y_pausar("Dimensiones incompatibles para suma.");
                }
                break;
            case 4:
                if (f == fB && c == cB) {
                    ...
                    mostrar_error_y_pausar("Dimensiones incompatibles para resta.");
                }
                break;
            case 5:
                if (c == fB) {
            ...
            case 6:
                if (f == c) {
                    ...
                    printf("\n");
                } else mostrar_error_y_pausar("A no es cuadrada.");
                if (fB == cB) {
                    printf("Determinante B: ");
                    imprimir_fraccion(determinante(B, fB), 12);
                    printf("\n");
                } else mostrar_error_y_pausar("B no es cuadrada.");
                break;
            case 7: {
                if (f == c) {
                    ...
                    } else mostrar_error_y_pausar("A no tiene inversa.");
                } else mostrar_error_y_pausar("A no es cuadrada.");
                if (fB == cB) {
                    ...
                    } else mostrar_error_y_pausar("B no tiene inversa.");
                } else mostrar_error_y_pausar("B no es cuadrada.");
                break;
            }
            case 8: {
                if (f == c) {
                    printf("Adjunta A:\n");
                    Fraccion** adjA = adjunta(A, f);
                    mostrar_matriz_color(adjA, f, f, 10);
                    liberar_matriz(adjA, f);
                } else mostrar_error_y_pausar("A no es cuadrada.");
                if (fB == cB) {
                    printf("Adjunta B:\n");
                    Fraccion** adjB = adjunta(B, fB);
                    mostrar_matriz_color(adjB, fB, fB, 12);
                    liberar_matriz(adjB, fB);
                } else mostrar_error_y_pausar("B no es cuadrada.");
                break;
            }
            case 9:
                if (f == c)
                    printf("Traza A: "), imprimir_fraccion(traza(A, f), 10), printf("\n");
                else mostrar_error_y_pausar("A no es cuadrada.");
                if (fB == cB)
                    printf("Traza B: "), imprimir_fraccion(traza(B, fB), 12), printf("\n");
                else mostrar_error_y_pausar("B no es cuadrada.");
                break;
            case 10:
                if (f == c && fB == f && cB == 1) {
                    Fraccion** X = resolver_sistema(A, B, f);
                    if (X) {
                        printf("Solución X (AX = B):\n");
                        mostrar_matriz_color(X, f, 1, 11);
                        liberar_matriz(X, f);
                    } else {
                        mostrar_error_y_pausar("A no tiene inversa, no se puede resolver.");
                    }
                } else mostrar_error_y_pausar("Dimensiones inválidas para sistema AX = B.");
                break;
            case 0:
                despedida();
                break;
